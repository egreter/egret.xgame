...
module xgame {
    ...
    export class MathUtils {
        ...
        */
         ...
         */
        public static map(value: number, minA: number, maxA: number, minB: number = 0, maxB: number = 1): number {
            var t: number = MathUtils.clamp01(MathUtils.inverseLerp(minA, maxA, value));
         ...
         * @return Rectangle
         */
        public static createAABB(x: number, y: number, w: number, h: number, a: number = 0): Rectangle {
         ...
         */
        public static createAABBData(x: number, y: number, w: number, h: number, a: number = 0, aabbdata: AABBData = null): AABBData {
            if (aabbdata == null) {
         ...
         */
        public static linesIntersection(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, x4: number, y4: number, asSegments: Boolean = true): Point {
            var ip: Point;
        ...
        public static getNextInArray(currentElement: any, array: any[]): any {
            var currIndex: number = array.lastIndexOf(currentElement) + 1;
         ...
         * @return
         */
        public static getRandomColor(minLum: number = 0, maxLum: number = 0xFF, b32: Boolean = false, randAlpha: Boolean = false): number {
            maxLum = maxLum > 0xFF ? 0xFF : maxLum;
            minLum = minLum > 0xFF ? 0xFF : minLum;

            var r: number = MathUtils.randomInt(minLum, maxLum);
            var g: number = MathUtils.randomInt(minLum, maxLum);
            var b: number = MathUtils.randomInt(minLum, maxLum);

            if (!b32)
                return r << 16 | g << 8 | b;
            else {
                var a: number = randAlpha ? MathUtils.randomInt(0, 255) : 255;
        ...
        public static colorLerp(fromColor: number, toColor: number, t: number): number {
            var q: number = 1 - t;
            var fromA: number = (fromColor >> 24) & 0xFF;
            var fromR: number = (fromColor >> 16) & 0xFF;
            var fromG: number = (fromColor >> 8) & 0xFF;
            var fromB: number = fromColor & 0xFF;
            var toA: number = (toColor >> 24) & 0xFF;
            var toR: number = (toColor >> 16) & 0xFF;
            var toG: number = (toColor >> 8) & 0xFF;
            var toB: number = toColor & 0xFF;
            var resultA: number = fromA * q + toA * t;
            var resultR: number = fromR * q + toR * t;
            var resultG: number = fromG * q + toG * t;
            var resultB: number = fromB * q + toB * t;
            var resultColor: number = resultA << 24 | resultR << 16 | resultG << 8 | resultB;
            return resultColor;
        }
        ...
        public static logx(val: number, base: number = 10): number {
         ...
         * c = end
         */
        public static evaluateQuadraticCurve(a: number, b: number, c: number, t: number = 0): number {
         ...
         * d = end
         */
        public static evaluateCubicCurve(a: number, b: number, c: number, d: number, t: number = 0): number {
