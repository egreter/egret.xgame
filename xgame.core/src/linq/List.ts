...
module xgame {
    export type TPredicate<T> = (value?: T, index?: number, list?: T[]) => boolean

    export class List<T> extends XObject {
         ...
         * 确定序列的所有元素是否满足一个条件。
         */
        public all(predicate: TPredicate<T>): boolean {
         ...
         */
        public any(): boolean
        public any(predicate: TPredicate<T>): boolean
        public any(predicate?: TPredicate<T>): boolean {
            return predicate
         ...
         */
        public count(): number
        public count(predicate: TPredicate<T>): number
        public count(predicate?: TPredicate<T>): number {
         ...
         */
        public first(): T
        public first(predicate: TPredicate<T>): T
        public first(predicate?: TPredicate<T>): T {
            if (this.count()) {
        ...
        public firstOrDefault(predicate: TPredicate<T>): T
        public firstOrDefault(predicate?: TPredicate<T>): T {
        ...
        public every(action: (value?: T, index?: number, list?: T[]) => boolean): boolean {
            return this._elements.every(action);
        }
        public filter(action: (value?: T, index?: number, list?: T[]) => boolean): T[] {
         ...
         */
        public last(): T
        public last(predicate: TPredicate<T>): T
        public last(predicate?: TPredicate<T>): T {
            if (this.count()) {
                return predicate
        ...
        public lastOrDefault(predicate: TPredicate<T>): T
        public lastOrDefault(predicate?: TPredicate<T>): T {
         ...
         * 删除与指定谓词定义的条件匹配的所有元素。
         */
        public removeAll(predicate: TPredicate<T>): List<T> {
         ...
         */
        public single(predicate?: TPredicate<T>): T {
         ...
         */
        public singleOrDefault(predicate?: TPredicate<T>): T {
         ...
         */
        public skipWhile(predicate: TPredicate<T>): List<T> {
            return this.skip(
         ...
         * 返回序列中的元素，只要指定的条件为真。
         */
        public takeWhile(predicate: TPredicate<T>): List<T> {
            return this.take(
                this.aggregate(ac => (predicate(this.elementAt(ac)) ? ++ac : ac), 0)
            )
        }
         ...
         * 基于谓词过滤一系列值。
         */
        public where(predicate: TPredicate<T>): List<T> {
